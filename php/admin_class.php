...
class Action
{
	...
	function save_order()
	{
		...
		//echo "<script>console.log('no products: " . count($_POST["product"]) . "' );</script>";
		...
		/*$sql = "SELECT MAX(order_id) AS 'id' FROM orders";
					...
					if ($number_of_products > 0) {
						for ($i = 0; $i < $number_of_products; $i++) {
							...
							$save = $this->db->query("INSERT INTO tenants set $data2");
						}
						echo "Data Inserted";
					}*/
		...
		$query = "INSERT INTO `orders` (`order_id`, `customer_id`, `order_date`, `delivery_date`, `amount_paid`, `delivered`, `csr`) VALUES (NULL, '" . $_POST['customer_id'] . "', current_timestamp(), NULL, '" . $_POST['amt_paid'] . "', 'n', '{$_SESSION['login_id']}');";
	...
	function save_customer()
	{
		...
		if (empty($customer_id)) {
			...
			$save = $this->db->query("UPDATE customers set $data where customer_id = $customer_id");
		}
		if ($save)
			return 1;
	}
